//Final Week To-Do List (1) Finalize Rules (2) Ask about calling in spaceships into splash screen (3) Consider adding a pause button and reset button 

//Global Variables
var currentScene = 0; //variable to control scene switches
...
var aliensLanded = 0; // turret health effected by ships touching the ground
...
var backdrop = function() {
    background(175, 210, 230);        //sky
    rectMode(CENTER);
    
    drawBitmojiMatt(20,376,44);       //bitmojis
    noStroke();
    drawBitmojiSam(71,356);
    fill(255, 255, 255);
   
    arc(83,80,77,55,-180,0);          //clouds
    arc(107,80,62,34,-180,0);
    arc(338,118,77,55,-180,0);
    arc(308,118,62,34,-180,0);
    arc(234,186,96,72,-180,0);
    arc(194,186,103,54,-180,0);
    arc(63,250,96,72,-180,0);
    arc(106,250,103,54,-180,0);
    arc(244,273,66,44,-180,0);
    arc(183,351,44,30,-180,0);
    arc(214,351,66,44,-180,0);
    arc(255,37,77,52,-180,0);
    arc(290,37,77,32,-180,0);
    
    //fortress
    image(getImage("cute/StoneBlock"),-8,308,18,61);
    image(getImage("cute/StoneBlock"),-8,325,18,61);
    image(getImage("cute/StoneBlock"),-8,346,18,61);
    image(getImage("cute/StoneBlock"),9,301,33,33);
    image(getImage("cute/StoneBlock"),41,301,33,33);
    image(getImage("cute/StoneBlock"),64,301,33,33);
    image(getImage("cute/StoneBlock"),94,308,18,61);
    image(getImage("cute/StoneBlock"),94,325,18,61);
    image(getImage("cute/StoneBlock"),94,353,18,61);
    image(getImage("cute/WallBlockTall"),81,292,31,34);
    image(getImage("cute/WallBlockTall"),-14,292,31,34     );
    image(getImage("cute/StoneBlock"),335,301,33,33);
    image(getImage("cute/StoneBlock"),366,301,36,33);
    image(getImage("cute/StoneBlock"),335,308,18,61);
    image(getImage("cute/StoneBlock"),335,325,18,61);
    image(getImage("cute/StoneBlock"),335,353,18,61);
    image(getImage("cute/WallBlockTall"),335,277,31,34);  image(getImage("cute/ChestClosed"),364,359,31,35);
    image(getImage("cute/CharacterPrincessGirl"),364,264,38,56);

    
    for(var k = 0; k<15;k++) {          //grass
    image(getImage("cute/GrassBlock"),k*29+-6,382,31,38     );  
    }
};

//Spaceship Object Class
//Spaceship Constructor Function
var spaceShip = function(x, y, size, isAnimated) {  
    this.x = x;
    this.y = y;
    this.size = size;
...
spaceShip.prototype.draw = function() {
    ...
    if (colorRandomizer === 0 && this.isAnimated) {fill (spaceshipColors [ 0 ] );}
    if (colorRandomizer === 1 && this.isAnimated) {fill (spaceshipColors [ 1 ] );}    //fills random color
    if (colorRandomizer === 2 && this.isAnimated) {fill (spaceshipColors [ 2 ] );}
...
Turret.prototype.draw = function() {
    ...
    rect(this.x,this.y-52,11,37);                     //turret
...
Turret.prototype.aliensLanded = function() {
    fill(255, 0, 0);
    text("Aliens Landed : " + aliensLanded, 10, 54);
...
Button.prototype.draw = function() {
    ...
    textSize(23);
    textAlign(LEFT, TOP);
    text(this.label, this.x+22, this.y+this.height-37);
};
...
//Creates NEW turret from Object Class
var turret = new Turret(190, 395);
//Creates in NEW motherships from Object Class
var motherShip = new motherShip(-70,45,2);
//Creates NEW Splash Screen ships
var spaceShip1 = new spaceShip(315,295,1,false);
var spaceShip2 = new spaceShip(75,295,1,false);

//For Loop to fill array with randomly spaced ships
for (var i = 0; i < numShips; i++){
    ships.push(new spaceShip(random(20, 360), i * - 100, 1, true));
...
var splashScene = function(){
    background(158, 72, 72);
    fill(255, 255, 255);
    textSize(69);
    var f = createFont("fantasy");
    textFont(f);
    text("ALIEN ATTACK", 19, 50);
    textSize(18);
    text("Created by Matt and Sam", 110, 105);
    startButton.draw();//call start button
    textAlign(LEFT, CENTER);
    drawBitmojiMatt(133,368,50); //draw Matt Bitmoji
    drawBitmojiSam(243, 363); //draw Sam Bitmoji
    fill(255, 255, 255);
    textSize(28);
    text("RULES",21,145);
    fill(255, 255, 255);
    var q = createFont("monospace");
    textFont(q);
    textSize(17);
    text("- USE LEFT AND RIGHT ARROW KEYS TO MOVE",13,175);
    text("- HOLD SPACEBAR TO SHOOT",13,200);
    textSize(19);
    text("- DESTROY 50 SPACESHIPS TO ADVANCE",10,225);
    text("- DEFEAT THE MOTHERSHIP TO WIN!",10,250);
    spaceShip1.draw();
    spaceShip2.draw();
    spaceShip(325,200);
    spaceShip(303,54);
...
var gameScene = function(){
    ...
    textSize(20);
    fill(255, 0, 0);
    text("Aliens Destroyed : " + score, 10,25);
...
var bossScene = function(){
    ...
    textSize(20);
    fill(255, 0, 0);
    text("Mothership Health: " + motherShipHealth + " Hits Left", 25, 19);
    ...
    if(motherShipHealth === 0){
        ...
        text("You Saved the World!", 15, 150);
        noLoop();
    }
    ...
    if(mShip === 4 && motherShip.x === width - 50){
     ...
     text("GAME OVER",100,200);
     textSize(25);
     text("Earth has been Conquered", 35, 240);
    }
...
//Calls button methods to change scenes
